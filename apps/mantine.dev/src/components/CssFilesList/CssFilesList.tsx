...
import { ActionIcon, Code, CopyButton, Group } from '@bds-react/core';
...
export function CssFilesList() {
  const files = ['global.css', ...CSS_FILES_LIST.modules].map((file) => [
    file.replace('.css', ''),
    <Group>
      <Code style={{ whiteSpace: 'nowrap' }}>{`import '@bds-react/core/styles/${file}';`}</Code>
      <CopyButton value={`import '@bds-react/core/styles/${file}';`}>
        {({ copy, copied }) => (
          <ActionIcon
