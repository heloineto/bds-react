...
`@bds-react/core` package now exports [DirectionProvider](/styles/rtl) component, which should be used to configure the direction of the application.
...
- `TransferList` component is no longer available as a part of `@bds-react/core` package, instead you can implement it with [Combobox](/core/combobox) component ([example](/combobox?e=TransferList))
