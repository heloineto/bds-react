...
`@bds-react/core` package includes some global styles that are required for components to work correctly.
...
`@bds-react/core` package includes minimal CSS reset – it includes only basic styles required for components to work
...
`@bds-react/core` package includes the following `body` and `:root` elements styles:
...
`@bds-react/core` package includes the following static classes:
