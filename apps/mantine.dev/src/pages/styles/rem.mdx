...
`@bds-react/core` package exports `rem` and `em` function that can be used to convert `px` into `rem`/`em`:

```tsx
import { em, rem } from '@bds-react/core';
...
To convert `rem`/`em` to `px` use `px` function exported from `@bds-react/core`:

```tsx
import { px } from '@bds-react/core';
