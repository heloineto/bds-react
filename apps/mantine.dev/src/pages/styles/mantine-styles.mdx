...
Example with `@bds-react/core` package:

```tsx
import '@bds-react/core/styles.css';
```

By adding this import, you will have all styles of `@bds-react/core` components in your application.
...
exported styles from `@bds-react/core` package and additional instructions on [this page](/styles/css-files-list).
...
Note that individual component styles are available only for `@bds-react/core` package.
Other packages have minimal styles that can be imported with `@mantine/{package}/styles.css` import.

## Styles import order

It is important to keep the correct styles import order. `@bds-react/core` package
...
```tsx
import '@bds-react/core/styles/UnstyledButton.layer.css';
import '@bds-react/core/styles/Button.layer.css';
...
<link
  rel="stylesheet"
  href="https://unpkg.com/@bds-react/core@7.4.2/styles.css"
/>

<!-- Styles with @layer directive -->
<link
  rel="stylesheet"
  href="https://unpkg.com/@bds-react/core@7.4.2/styles.layer.css"
/>
```
