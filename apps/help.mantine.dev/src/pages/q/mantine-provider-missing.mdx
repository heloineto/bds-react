...
<ErrorMessage error="@bds-react/core: MantineProvider was not found in component tree, make sure you have it in your app." />
...
- You have different versions of `@mantine/*` packages in your application.
  For example, you have `@bds-react/core@7.0.0` and `@mantine/dates@7.1.0` installed.
- There was an issue during packages installation. Usually this happens with pnpm.
- Your app has multiple instances of `@bds-react/core` package in the bundle. This may
...
- Make sure that you have only one version of `@bds-react/core` in your app. If you are using
  monorepo, make sure all packages depend on the same version of `@bds-react/core`.
