...
```tsx
import { ColorSchemeScript, MantineProvider } from '@bds-react/core';
...
These styles are part of the `@bds-react/core` package and are applied automatically when you import
`@bds-react/core/styles.css` in your application. Note that these styles cannot be decoupled from the
rest of the library.
...
import { TextInput, MantineProvider, createTheme } from '@bds-react/core';
...
```tsx
import cx from 'clsx';
import { createTheme, MantineProvider, TextInput } from '@bds-react/core';
import classes from './Demo.module.css';
...
```tsx
import { Button, createTheme, Group, MantineProvider, rem } from '@bds-react/core';
...
```tsx
import { createTheme, Input, MantineProvider } from '@bds-react/core';
import classes from './Demo.module.css';
...
`@bds-react/core` package now exports [DirectionProvider](https://mantine.dev/styles/rtl) component, which should be used to configure the direction of the application.
...
- `AppShell` now uses compound components pattern: `Navbar`, `Aside`, `Header` and `Footer` are no longer exported from `@bds-react/core` package. Instead, use `AppShell.Navbar`, `AppShell.Aside`, etc.
...
```tsx
import { Fieldset } from '@bds-react/core';
...
- `TransferList` component is no longer available as a part of `@bds-react/core` package, instead you can implement it with [Combobox](https://mantine.dev/core/combobox) component ([example](https://mantine.dev/combobox?e=TransferList))
