...
both in `rem` function exported from `@bds-react/core` package and [postcss-preset-mantine](https://mantine.dev/styles/postcss-preset).
...
```tsx
import { Modal } from '@bds-react/core';
...
```tsx
import { createTheme } from '@bds-react/core';
...
- [mantine-flagpack](https://mantinedev.github.io/mantine-flagpack/) has been migrated to support `7.x` versions of `@bds-react/core`. To use it, update `mantine-flagpack` to `4.0.0`.
